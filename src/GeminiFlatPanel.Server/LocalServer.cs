//
// ASCOM.CCDASTRO.GeminiFlatPanel Local COM Server
//
...
namespace ASCOM.LocalServer
{
    public static class Server
    {
        ...
        private static string localServerAppId = "{cd0179f2-3945-4b8e-abaf-a519fc635ab9}"; // Our AppId
        ...
        static void Main(string[] args)
        {
            ...
            TL = new TraceLogger("", "CCDASTROGeminiFlatPanel.LocalServer")
            {
            ...
            Thread.CurrentThread.Name = "CCDASTROGeminiFlatPanel Local Server Thread";
        ...
        private static bool PopulateListOfAscomDrivers()
        {
            ...
            catch (Exception e)
            {
                ...
                MessageBox.Show($"Failed to load served COM class assembly from within this local server - {e.Message}", "Gemini FlatPanel", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return false;
            }
        ...
        /// </remarks>
        private static RegistryKey registrationRoot;
        private static void RegisterAllViews(bool remove)
        {
            if (!IsAdministrator) { ElevateSelf(remove ? "/unregister" : "/register"); return; }
            ...
            foreach (var view in views)
            {
                ...
                {
                    registrationRoot = classes;
                    if (remove) UnregisterObjects(); else RegisterObjects();
                }
            }
        ...
        private static void RegisterObjects()
        {
            ...
            try
            {
                ...
                using (RegistryKey appIdKey = registrationRoot.CreateSubKey($"APPID\\{localServerAppId}"))
                {
                ...
                using (RegistryKey exeNameKey = registrationRoot.CreateSubKey($"APPID\\{Application.ExecutablePath.Substring(Application.ExecutablePath.LastIndexOf('\\') + 1)}"))
                {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error while registering the server:\n" + ex.ToString(), "ASCOM.CCDASTRO.GeminiFlatPanel", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            ...
            foreach (Type driverType in driverTypes)
            {
                ...
                try
                {
                    ...
                    using (RegistryKey clsIdKey = registrationRoot.CreateSubKey($"CLSID\\{clsId}"))
                    {
                    ...
                    // HKCR\CLSID\progid
                    using (RegistryKey progIdKey = registrationRoot.CreateSubKey(progId))
                    {
        ...
        private static void UnregisterObjects()
        {
            ...
            registrationRoot.DeleteSubKey($"APPID\\{localServerAppId}", false);
            registrationRoot.DeleteSubKey($"APPID\\{Application.ExecutablePath.Substring(Application.ExecutablePath.LastIndexOf('\\') + 1)}", false);
            ...
            foreach (Type driverType in driverTypes)
            {
                ...
                registrationRoot.DeleteSubKey($"{progId}\\CLSID", false);
                registrationRoot.DeleteSubKey(progId, false);

                // Remove CLSID entries
                registrationRoot.DeleteSubKey($"CLSID\\{clsId}\\Implemented Categories\\{{62C8FE65-4EBB-45e7-B440-6E39B2CDBF29}}", false);
                registrationRoot.DeleteSubKey($"CLSID\\{clsId}\\Implemented Categories", false);
                registrationRoot.DeleteSubKey($"CLSID\\{clsId}\\ProgId", false);
                registrationRoot.DeleteSubKey($"CLSID\\{clsId}\\LocalServer32", false);
                registrationRoot.DeleteSubKey($"CLSID\\{clsId}\\Programmable", false);
                registrationRoot.DeleteSubKey($"CLSID\\{clsId}", false);

                using (var profile = new Profile()) { profile.DeviceType = driverType.Name; profile.Unregister(progId); }
            }
        }
        ...
        private static void ElevateSelf(string argument)
        {
            ...
            {
                TL?.LogMessage("IsAdministrator", $"The ASCOM.CCDASTRO.GeminiFlatPanel was not " + (argument == "/register" ? "registered" : "unregistered because you did not allow it."));
                MessageBox.Show("The ASCOM.CCDASTRO.GeminiFlatPanel was not " + (argument == "/register" ? "registered" : "unregistered because you did not allow it.", "ASCOM.CCDASTRO.GeminiFlatPanel", MessageBoxButtons.OK, MessageBoxIcon.Warning));
            }
            catch (Exception ex)
            {
                TL?.LogMessageCrLf("IsAdministrator", $"Exception: {ex}");
                MessageBox.Show(ex.ToString(), "ASCOM.CCDASTRO.GeminiFlatPanel", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
        ...
        private static bool RegisterClassFactories()
        {
            ...
            foreach (Type driverType in driverTypes)
            {
                ...
                if (!factory.RegisterClassObject())
                {
                    ...
                    MessageBox.Show("Failed to register class factory for " + driverType.Name, "ASCOM.CCDASTRO.GeminiFlatPanel", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
            ...
            if (args.Length > 0)
            {
                switch (args[0].ToLower())
                {
                    ...
                    case @"/regserver":
                        ...
                        RegisterAllViews(false); // Register each served object
                    ...
                    case @"/unregserver":
                        ...
                        RegisterAllViews(true); // Unregister each served object
                        returnStatus = false; // Terminate on return
                        break;

                    default:
                        TL?.LogMessage("ProcessArguments", $"Unknown argument: {args[0]}");
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding", "ASCOM.CCDASTRO.GeminiFlatPanel", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
